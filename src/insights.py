...
ORIG_COLS = ['lang', 'idx', 'orig', 'label', 'orig_truelabel_probs']
METRIC_COLS=dict()
METRIC_COLS['training']   = ['label_flip_frac', 'vm_truelabel_probs_mean', 'vm_truelabel_probs_quantiles',
       'vm_scores_mean', 'vm_scores_quantiles', 'sts_scores_mean', 'sts_scores_quantiles', 'ld_scores_mean', 'ld_scores_quantiles',
       'kl_divs_mean', 'kl_divs_quantiles']

METRIC_COLS['validation'] = METRIC_COLS['test'] = ['vm_truelabel_probs', 'vm_scores', 'sts_scores', 'ld_scores', 'kl_divs',
         'label_flip', 'is_valid_pp', 'is_adv_example', 'adv_score', 'quality_score', 'ref_logp', 'pp_logp' ]
PP_COLS = dict()
PP_COLS['training']   = ['global_step',          'pp']    
PP_COLS['validation'] = ['global_step', 'pp_idx','pp', 'pp_predclass']
PP_COLS['test']       = ['global_step', 'pp_idx','pp', 'pp_predclass']
...
def preprocess_df(df, split): 
    ...
    if evaluation_csv: cols = cols + ['flu', 'ent', 'langscore', 'sim', 'quality_score']
...
def print_interesting_text_stats_training_step(df, n, split): 
    def print_stats(df, idx_d, key, i):
        ...
        g = df1.groupby(g_fields).agg({'global_step' : lambda x: list(x)})
        g = g.sort_values(by='global_step', key = lambda col: col.map(lambda x: np.min(x)))
...
def show_random_examples_for_eval_set(df, split, n, global_step=0): 
    split = "test" if "test" in split else split
    assert global_step <= df.global_step.max()
    ...
    for i, idx in enumerate(random_idx): 
        ...
        print("random", i+1, "\n")
        if split == 'validation': 
            df1 = df.query("idx==@idx").query("global_step==@global_step")
        else: 
            df1 = df.query("idx==@idx")
        ...
        df3 = df2.groupby(['pp', 'pp_predclass']).agg({'global_step': lambda x: list(x), **agg_d})
        df3.sort_values(by ='global_step', key=lambda col: col.map(lambda x: np.min(x)), inplace=True)
        display_all(df3)
        print("Stats per validation epoch")
        df4 = df2.groupby('global_step').agg(agg_d)
