...
class timecode:
    ...
    Usage: 
    ```
        with timecode() as x:  print("test fn")
        print(x.t)  # has time
    ```
    """
    def __enter__(self):
...
def in_debug_mode(): return os.environ.get('DEBUG_MODE', '').lower() == 'true'
...
def display_all(df):
    ...
    with option_context("display.max_rows", 3000):
        with option_context("display.max_columns", 1000):
            with option_context("max_colwidth", 480):
                display(df)
...
def unpack_nested_lists_in_df(df, scalar_cols=[]):
    ...
    return df.set_index(scalar_cols).apply(Series.explode).reset_index()
...
def file_is_unchanged(fname, args): 
    with open(fname, "rb") as f: 
        current_checksum = md5(f.read()).hexdigest()    
    checksum_fname = f"{args.cache_dir}/{fname.split('/')[-1]}.checksum"
    if not path.exists(checksum_fname):
...
def gen_model_type(x):
    if "t5" in x.lower():  
        return "mt5" if 'mt5' in x.lower() else "t5"
    raise Exception("Generative model must be either T5 or mT5")
